...
-- variable via an anonymous ROW() constructor:
--
--   v_params := row(v_existing.rendimiento_aplicado, ...);
--
-- A plpgsql record populated this way has no field names (Postgres cannot
-- infer them from a bare ROW() expression), so the later field lookups
-- (v_params.rendimiento_aplicado, etc.) raise "record ... has no field"
-- at runtime. This only ever fires on the freeze-rule path itself — the
-- very first save of a trip (or any borrador save) takes the other branch,
-- which populates v_params via `select * from
-- obtener_parametros_liquidacion(...)` and does carry field names, so it
-- never surfaced until a second guardar_viaje call was made against an
-- already-liquidado trip.
--
-- Replaces the single `record` with four scalar variables that are
-- populated on both branches, so field access is never ambiguous.
